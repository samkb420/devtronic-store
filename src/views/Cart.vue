<template>
   <div class="emb-cart-wrap emb-card">
      <emb-page-title
         ...
         subHeading="Click on Checkout, or go back to add more items to the cart."
         >
      </emb-page-title>
							...
							<v-row class="cart-item-wrap">
								...
								<v-col cols="12" sm="6" md="4" class="d-inline-flex align-center justify-center">
									<div class="text-center">
										...
										<p class="mb-0">{{ (item.description || '').substring(0, 70) }}</p>
									</div>
								</v-col>
...
export default {
  ...
  methods: {
    ...
    onDeleteProductFromCart() {
      ...
      this.$snotify.error("Removing from cart", {
        closeOnClick: false,
        pauseOnHover: false,
        timeout: 1000
      });
