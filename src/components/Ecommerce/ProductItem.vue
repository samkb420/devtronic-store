	...
	<div class="product-item-wrap emb-card">
		<div>
			<div class="thumb-warp" >
				<router-link to="">
					<img alt="product" style="height: 210px !important;" :src="data.image" >
				</router-link>
				...
				<div class="add-to-cart">
					...
					<v-btn v-else class="accent" small icon @click="addProductToCart(data)" >
				...
				<div class="emb-meta-info layout align-center justify-space-between">
					<div class="inline-block">
						<h6 class="accent--text">
							Ksh {{parseFloat(data.price).toFixed(2)}}
						</h6>
					</div>
				</div>
			</div>
	...
	methods: {
		...
		*/
		addProductToCart(item) {
			this.$snotify.success('Product was added to your cart',{
				closeOnClick: false,
				pauseOnHover: false,
				timeout: 1000,
		...
		*/
		...
		addItemToWishlist(data) {
			if(this.ifItemExistInWishlist(data)) {
				this.$snotify.error('Product already exists in your wishlist',{
					showProgressBar:false,
				});
			} 
			else {
				this.$snotify.success('Product was added to your wishlist',{
					closeOnClick: false,
					pauseOnHover: false,
					timeout: 1000,
					showProgressBar:false,
